module.exports = function(module, Parse, needsComponent) {
  ...
  var view = function(controller) {
    return m('section#accounts', [m(".container", [
      ...
      button('Add Account', modal({
        content: m('div[style="width:500px"]', [
          m("label[for='name']", "Account Name"),
          m("input#name", {
            onchange: m.withAttr("value", module.$.newAccount
              .account_name),
        ...
        onConfirm: function() {
          if (module.$.newAccount.account_name().length >
            0) {
      ...
      m("table.striped.accounts", [
        ...
        m("tbody", [
          module.$.accounts() && module.$.accounts().map(
            function(account, index) {
              return m("tr", [
                m("td", account.get('account_name')),
                m("td", "$" + parseFloat(account.get(
                  'balance')).toFixed(2)),
                m("td.control", [
                  button("View", '/accounts/view/' +
                    account.id, {
                      class: 'waves-effect'
                    })
                ])
